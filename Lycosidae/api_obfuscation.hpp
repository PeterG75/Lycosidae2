...
{
	const auto _hash = t1ha0((LPCSTR)PRINT_HIDE_STR("CreateFileA"), strlen((LPCSTR)PRINT_HIDE_STR("CreateFileA")),
	                         STRONG_SEED);
...
{
	const auto _hash = t1ha0((LPCSTR)PRINT_HIDE_STR("GetModuleHandleA"),
	                         strlen((LPCSTR)PRINT_HIDE_STR("GetModuleHandleA")), STRONG_SEED);
	temp_GetModuleHandleA = static_cast<HMODULE(WINAPI *)(LPCSTR)>(get_api(
		_hash, (LPCSTR)PRINT_HIDE_STR("kernel32.dll"), strlen((LPCSTR)PRINT_HIDE_STR("GetModuleHandleA")),
		STRONG_SEED));
...
{
	const auto _hash = t1ha0((LPCSTR)PRINT_HIDE_STR("GetLastError"), strlen((LPCSTR)PRINT_HIDE_STR("GetLastError")),
	                         STRONG_SEED);
                                    ...
                                    LPDWORD lpReturnLength)
{
	...
	temp_QueryInformationJobObject = static_cast<BOOL(*)(HANDLE,
	                                                     ...
	                                                     DWORD,
	                                                     LPDWORD)>(get_api(
		_hash, (LPCSTR)PRINT_HIDE_STR("kernel32.dll"),
		strlen(func), STRONG_SEED));
                           ...
                           _In_ PCLIENT_ID ClientId)
{
	...
	temp_NtOpenThread = static_cast<NTSTATUS(*)(_Out_ PHANDLE,
	                                            ...
	                                            _In_ PCLIENT_ID)>(get_api(
		_hash, (LPCSTR)PRINT_HIDE_STR("ntdll.dll"), strlen(func),
		STRONG_SEED));
                                  ...
                                  ULONG FreeType)
{
	...
	temp_NtFreeVirtualMemory = static_cast<NTSTATUS(*)(HANDLE,
	                                                   PVOID*,
	                                                   PSIZE_T,
	                                                   ULONG)>(get_api(_hash, (LPCSTR)PRINT_HIDE_STR("ntdll.dll"),
	                                                                   strlen(func),
	                                                                   STRONG_SEED));
